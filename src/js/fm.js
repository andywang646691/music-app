   //生成HTML
    var FmHtml = function ($node) {
        this.$node = $node;
        this.createHtml();
    };

    FmHtml.prototype.createHtml = function () {
        this.$node.append(
...
var Fm = (function () {
    function _Fm($node) {
        ...
        this.$timeLine = this.$node.find(".control .time-line");//时间线
    ...
    _Fm.prototype = {
        ...
        bind: function () {
            ...
            _this.$left.on("click mouseenter", function () {
                _this.$fmChannel.addClass("active");
            });
            _this.$right.on("click mouseenter", function () {
                _this.$fmList.addClass("active");
            });
            ...
            _this.$channels.on("click", "li", function () {
                var idx = $(this).index(),
                    chArr = _this.$channels.children();
                if (idx !== 0) {
                    channelId = $(this).attr("data-id");
                    channelName = $(this).text();
                    chArr.removeClass("active");
                    $(this).addClass("active");
            ...
            _this.$swithLrc.on("click", function () {
                ...
                if ($this.hasClass("active")) {
                    _this.lrcWrap.removeClass("active");
                    _this.$swithLrc.removeClass("active");
                    _this.$background.removeClass("active");
                    // _this.$node.height("173px")
                } else {
                    _this.lrcWrap.addClass("active");
                    _this.$swithLrc.addClass("active");
                    _this.$background.addClass("active");
                    // _this.$node.height("482.95px")
                }
            });
            _this.$node.on('mousedown', function (e) {
                var $this = $(this),
                    newL = e.pageX - $this.offset().left,
                    newT = e.pageY - $this.offset().top,
                    maxX = $(window).width() - $this.outerWidth(),
                    maxY = $(window).height() - $this.outerHeight();
                $(document).on('mousemove', function (e) {
                    var lastX = e.pageX - newL,
                        lastY = e.pageY - newT;
                    if (lastX < 0) {
                        lastX = 0
                    }
        ...
        shuffleMusic: function () {
            var _this = this,
                can = false,
                channelArr = _this.$channels.children();
            if (channelArr.eq(0).hasClass("active")) {
                (function () {
                    _this.randomCH();
                    can = true;
                })();
